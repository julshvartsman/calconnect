/**
 * Shared seed logic — runs when the database has no resources.
 * Used by /api/seed and auto-seeded when Browse page loads empty data.
 */
...
/**
 * Seeds the database with Berkeley resources if empty.
 * Returns true if seeding ran, false if already had data.
 */
export async function runSeedIfNeeded(): Promise<boolean> {
  ...
  for (const r of RESOURCES) {
    ...
    const existingResource = await prisma.resource.findFirst({ where: { name: r.name } });
